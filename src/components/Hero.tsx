...
const words = ["We", "Automate", "the", "Chaos."];
  ...
  return (
    <section ref={ref} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      >
        <div className="animate-blob relative">
          ...
          <div
            ...
            style={{
              inset: "-80px",
              background: "radial-gradient(circle, rgba(0,232,135,0.12) 0%, rgba(0,194,255,0.06) 50%, transparent 75%)",
              filter: "blur(60px)",
            }}
          />
          {/* Main sphere body */}
          <div
            className="w-[480px] h-[480px] sm:w-[600px] sm:h-[600px] rounded-full relative"
            style={{
              background:
                "radial-gradient(circle at 36% 34%, rgba(180,255,220,0.95) 0%, rgba(0,232,135,0.85) 18%, rgba(0,194,255,0.6) 45%, rgba(0,60,180,0.4) 68%, rgba(0,5,20,0.95) 88%)",
              boxShadow:
                "0 0 120px rgba(0,232,135,0.3), 0 0 250px rgba(0,232,135,0.12), inset 0 0 80px rgba(0,0,0,0.5)",
            }}
          >
            {/* Specular highlight */}
            <div
      ...
      >
        ...
        >
          {words.map((w, i) => (
            <motion.span key={i} variants={word} className={i === 3 ? "text-gradient" : ""}>
              {w}
            </motion.span>
          ))}
        </motion.h1>
        ...
        >
          ...
          <a
            href="#services"
            className="flex items-center gap-2 px-8 py-4 rounded-full btn-ghost text-sm font-medium"
          >
            See What We Build
          </a>
        </motion.div>
